...
<script>
import { accountEdit, accountEditSubmit, accountgetAll } from "../../../api";
export default {
  data() {
    return {
      accFrom: {
  ...
  methods: {
    ...
    getIdData() {
      accountEdit(this.$route.query.id).then((res) => {
        (this.accFrom.label = res.data.data.label),
          (this.accFrom.code = res.data.data.code),
          (this.accFrom.url = res.data.data.url),
          (this.accFrom.type = res.data.data.type),
          (this.accFrom.parentId = res.data.data.parentId);
      });
    },
    goBack() {
      this.$router.go(-1);
