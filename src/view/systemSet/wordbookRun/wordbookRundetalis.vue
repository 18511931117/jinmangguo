...
<script>
import { wordEdit, wordEditSubmit } from "../../../api";
export default {
  data() {
    return {
      wordFrom: {
  ...
  methods: {
    getIdData() {
      wordEdit(this.$route.query.id).then((res) => {
        (this.wordFrom.name = res.data.data.name),
          (this.wordFrom.type = res.data.data.type),
          (this.wordFrom.key = res.data.data.key),
          (this.wordFrom.value = res.data.data.value);
      });
    },
    goBack() {
      this.$router.go(-1);
