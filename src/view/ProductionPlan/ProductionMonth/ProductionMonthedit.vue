<template>
  <div>
    <el-page-header @back="goBack" content="月计划编辑"> </el-page-header>
    <div class="warp">
    <div class="resulting">
    ...
    <div class="picklisttable">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="印刷工序" name="first" :key="'first'">
          <ProductionMonthTabsFirst :data="data" :monthname="id" ref="ProductionMonthTabsFirst"></ProductionMonthTabsFirst>
        </el-tab-pane>
        <el-tab-pane label="烫金工序" name="second" :key="'second'">
          <ProductionMonthTabsSecond :data="data" :monthname="id" ref="ProductionMonthTabsSecond"></ProductionMonthTabsSecond>
        </el-tab-pane>
        <el-tab-pane label="分切工序" name="third" :key="'third'">
          <ProductionMonthThird :data="data" :monthname="id" ref="ProductionMonthThird"></ProductionMonthThird>
        </el-tab-pane>
        <el-tab-pane label="打孔工序" name="fourth" :key="'fourth'">
          <ProductionMonthFourth :data="data" :monthname="id" ref="ProductionMonthFourth"></ProductionMonthFourth>
        </el-tab-pane>
        <el-tab-pane label="复卷工序" name="five" :key="'five'">
          <ProductionMonthTabsFive :data="data" :monthname="id" ref="ProductionMonthTabsFive"></ProductionMonthTabsFive>
        </el-tab-pane>
      </el-tabs>
    </div>
    <el-row class="row">
...
<script>
  import {
    monthget,updateMonthPlaninsertMonthPlan
  } from "../../../api";
    ...
    methods: {
      ...
      init(){
        monthget(this.$route.query.id).then(res => {
          ...
          this.data = [res.data.data.startDate,res.data.data.endDate];
        })
      },
      onSubmit() {
        var submitData = [];
        ...
        if(this.monthname != "" && this.data != ""){
          var subData = {'id':this.id,'name':this.monthname,'unit':'t','startDate':this.data[0],'endDate':this.data[1],'details':submitData};
       
        updateMonthPlaninsertMonthPlan(subData).then((res) => {
          this.$router.go(-1);
        });
        }else{
          this.$message({
