...
<script>
  import {
    devicegetAll,
    productinfogetAll,
    monthPlaninsertMonthPlan, monthget, noticeget,
  } from "../../../api";
  export default {
    ...
    created() {
      ...
      if(this.$route.query.id != null && this.$route.query != undefined){
        noticeget(this.$route.query.id).then(res => {
          var deviceVal1 = {};
          var details = res.data.data.detaileds;
          for (var i = 0;i<details.length;i++){
            if(details[i].processId == '1'){
              if(deviceVal1[details[i].deviceId] == null || deviceVal1[details[i].deviceId] == undefined){
                deviceVal1[details[i].deviceId] = [];
              }
              deviceVal1[details[i].deviceId].push({'id':details[i].productId,'name':details[i].productName,'productNum':details[i].productNum});
            }
          }
