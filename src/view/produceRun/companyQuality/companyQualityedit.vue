  ...
  <div>
    ...
    <div class="xd_table">
      ...
      >
        ...
        ></vxe-table-column>
        <vxe-table-column field="sumPoints" title="得分" width="80" :edit-render="{
                name: 'input',
                immediate: true,
                attrs: { type: 'text' },
              }">
...
<script>
import { qualityCompanyFinishDetailsgetAll ,qualityCompanyFinishDetailsupdate,qualityCompanyFinishget,qualityCompanyFinishDetailsgetPage} from "../../../api";
export default {
  data() {
    return {
      name: "",
  ...
  methods: {
    init() {
      qualityCompanyFinishDetailsgetPage({
        page:1,
        limit:1000,
        companyFinishId:this.$route.query.id
      }).then((res) => {
     ...
     editClosedEvent ({ row, column }) {
               var totalrow= this.countAmount(row)
